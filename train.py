# -*- coding: utf-8 -*-
"""
Created on Sun Aug  8 03:04:52 2021
...
def run():
    ...
    for json in json_list:
        cnt += 1
        file_name, object_name, bbox = json_parser(json)
        print(f'{cnt} / {tot_num_json}')


    dataset = AiHub(json_list, tot_num_json)
    ...
    num_classes = len(label_dic)  # 12 개 클래스
    ...
    model = torchvision.models.detection.FasterRCNN(backbone, num_classes=12)
    ...
    box_predictor = torchvision.models.detection.faster_rcnn.FastRCNNPredictor(4096, 12)  # 12 class
    ...
    # train_dataset, val_dataset = random_split(dataset, [8571, 2142],
    ...
    # training
    #
    len_data = len(dataset)  # voc2007은 5011개 이미지
    term = 10
    try:
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        #
        # 이부분 training / validation 으로 나누기 -> error
        #
        #dataset = AiHub(json_list[:len_data], len_data - start_idx)
        ...
        for i, (image, targets) in enumerate(dataloader, start_idx):
            ...
            if (i + 1) % term == 0:
                ...
                print("Epoch {} | Iter {}/{} | Loss: {} | Duration: {} sec".format(epoch, (i + 1), len(dataloader),
                                                                                   (loss_sum / term), int((end - start))))
...
def preprocessing(root_dir):
    ...
    for file in files:
        ...
        file_name = path
        # print(file_name.split('\\')[4].split('.')[0])
        #print(path)
        # JSON 파일읽기
